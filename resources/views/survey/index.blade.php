...
@section('content')
    ...
    <form action="{{ route('survey.store') }}" class="mt-4 flex flex-col" method="post">
        <div class="flex h-full w-full flex-row justify-between">
            <button type="submit" class="rounded bg-emerald-500 px-3 py-1 font-semibold text-white transition-all hover:bg-white hover:text-emerald-800 hover:outline hover:outline-2 hover:outline-emerald-500" type="submit">Terminar</button>
        ...
        @csrf
        <div class="flex h-fit w-full mt-4 flex-col rounded-md  py-2">
            @foreach ($quest as $item)
                <fieldset class=" bg-white first:mt-0 last:mb-0 my-[1.2rem] flex h-fit w-full flex-col rounded-sm  px-2 py-4 shadow-lg">
    ...
    <div class="flex flex-row justify-between flex-1">
        <a class="mr-4 border border-red-400 px-2 py-0.5 rounded-sm" href="#">Voltar</a>
        <a class="ml-4 border border-emerald-400 px-2 py-0.5 rounded-sm" href="#">Continuar</a>
    </div>
@endsection
